...
export class Engine {
  ...
  token: string = "";
  startPlayTime: number = 0; // 当前时间戳  ms
  ...
  async startProcessAudio() {
    ...
    await this.audioContext.audioWorklet.addModule(
      "https://y-dev.tuwan.com/pcm-processor.js"
    );
  ...
  async genBgmTracks(songId: string, isAccompany?: boolean) {
    ...
    if (mp3Data[0]) {
      // 伴奏
      let track1 = await manager.createMediaStreamTrackFromMp3(
        mp3Data[0],
        "accompany"
      );
      let audioTrack = await AgoraRTC.createCustomAudioTrack({
        mediaStreamTrack: track1.mediaTrack,
      });
    ...
    if (mp3Data[1]) {
      // 原唱
      let track2 = await manager.createMediaStreamTrackFromMp3(
        mp3Data[1],
        "original"
      );
      let rtrack2 = await AgoraRTC.createCustomAudioTrack({
        mediaStreamTrack: track2.mediaTrack,
      });
  ...
  playBgm(bgmType: BgmType) {
    ...
    this.startPlayTime = manager.audioContext.currentTime;
  ...
  getBgmProgress() {
    ...
    const track =
      this.bgmType === BgmType.ORIGINAL
        ? this.originalBgmTrack
        : this.accompanyBgmTrack;
    const duration = track.status.duration;
    time = (manager.audioContext.currentTime - this.startPlayTime) % duration;
